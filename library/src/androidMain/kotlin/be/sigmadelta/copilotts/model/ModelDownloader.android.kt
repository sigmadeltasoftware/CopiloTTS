...
/**
 * Android implementation of model downloader using Ktor.
 */
actual class ModelDownloader actual constructor(
    ...
    actual fun downloadModel(
        ...
        if (availableSpace < modelInfo.sizeBytes * 1.2) { // 20% buffer
        ...
        try {
            ...
            val outputDir = File(modelStorage.getStorageDirectory(), modelId.replace("/", "_"))
            outputDir.mkdirs()
            ...
            val modelPath = if (modelInfo.isMultiFileModel) {
                ...
                File(outputDir, "model.onnx").absolutePath
            }
    ...
    private suspend fun downloadFile(
        ...
        var totalBytesDownloaded = currentOffset
    ...
    private fun buildDownloadUrl(modelInfo: ModelInfo, quantization: QuantizationType): String {
        // Build URL based on model architecture and quantization
